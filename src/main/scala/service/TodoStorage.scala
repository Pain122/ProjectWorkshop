...
object TodoStorage {
  ...
  private final class LoggingImpl(storage: TodoStorage[IO])(implicit
      logging: Logging[IO]
  ) extends TodoStorage[IO] {
      ...
      info"$inputLog" *> io.flatTap {
        case Left(error) =>
          ...
          throwable.fold(error"$logString")(err =>
            errorCause"$logString".apply(err)
          )
    ...
    override def create(
        todo: CreateTodo
    ): IO[Either[AppError, Todo]] = surroundWithLogs(storage.create(todo))("Creating Todo")(err => (err.message, err.cause))(success => s"Created todo: $success")
  ...
  def make(
    ...
    implicit val logging: Id[Logging[IO]] = logs.forService[TodoStorage[IO]]
